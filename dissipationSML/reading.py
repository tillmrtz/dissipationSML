...
def convert_with_variables(datasets, variables_needed):
    ...
    ### Filter the dataset to keep only the needed variables
    ds = ds[variables_needed]

    # Rename different variables
    ds = ds.rename_vars({'divenum': 'DIVE_NUMBER','GLIDER_VERT_VELO_MODEL': 'W_MODEL',
                          'GLIDER_HORZ_VELO_MODEL': 'U_MODEL'})
